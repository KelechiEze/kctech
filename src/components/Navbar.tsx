          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="flex items-center gap-1 text-sm font-medium transition-colors hover:text-brand text-white"
              >
                {link.name}
              </a>
            ))}
            ...
            <div className="flex flex-col gap-8">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  href={link.href}
                  onClick={() => setIsMenuOpen(false)}
                  className="text-4xl font-display font-bold text-white hover:text-brand transition-colors flex items-center justify-between group"
                >
                  {link.name}
               ...
               >
                 <svg viewBox="0 0 100 100" className="w-full h-full">
                   ...
                   <text className="text-[10px] font-bold uppercase tracking-[4px] fill-brand">
                     <textPath xlinkHref="#mobileCirclePath">
